    ...
    >
      {badges.map(badge => (
        <li key={badge}>
          <Badge
            variant="secondary"
            className="align-middle text-xs print:px-1 print:py-0.5 print:text-[8px] print:leading-tight dark:bg-neutral-600"
          >
            {badge}
          </Badge>
        </li>
...
function WorkPeriod({ start, end }: WorkPeriodProps) {
  return (
    <div
      className="text-sm tabular-nums text-gray-500 dark:text-neutral-400"
...
function WorkExperienceItem({ work }: WorkExperienceItemProps) {
  const { company, link, modeOfWork, badges, title, start, end, description } =
    work;
      ...
      <CardHeader className="print:space-y-1">
        <div className="flex items-center justify-between gap-x-2 text-base">
          <h3 className="inline-flex items-center justify-center gap-x-1 font-semibold leading-none print:text-sm">
            ...
            <BadgeList className="gap-x-1" badges={modeOfWork} />
          </h3>
